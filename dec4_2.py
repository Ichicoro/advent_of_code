inputstr = '''[1518-05-30 00:04] Guard #2417 begins shift
...
inputstra = '''[1518-11-01 00:00] Guard #10 begins shift
...
list = [
	{
		'date': date,
		'a': either -1 (sleep), 0 (wake) or guard number
	}
]
'''
...
def parseInput():
	...
	for line in inputstr.splitlines():
		timetext = line[1:17]
		#print(timetext)
		time = datetime.strptime(timetext, '%Y-%m-%d %H:%M')
		line = line[19:]
		if 'Guard' in line:
			guard = get_guard(line)
		elif 'falls' in line:
			prep_day(time.__format__('%m-%d'), guard)
			last_time = time.minute
		elif 'wakes' in line:
			try:
				for i in range(last_time, time.minute+1):
					data[time.__format__('%m-%d')]['sleeping'][i] = 1
			except KeyError:
				pass
...
def reorder():
	...
	ulist.sort(key=lambda x:x['date'])
	inputstr = ""
	for x in ulist:
		inputstr += f'[{x["date"].__format__("%Y-%m-%d %H:%M")}] '
		if x['a'] == -1:
			inputstr += 'falls asleep'
		elif x['a'] == 0:
			inputstr += 'wakes up'
		else:
			inputstr += f'Guard #{x["a"]} begins shift'
		inputstr += '\n'
	# print(inputstr)
...
def sleepiest_minute():
	...
	counts = dict()
	for day in data.values():
		counts[day['guard']] = [0]*60
	for day in data.values():
		for i in range(0,59):
			counts[day['guard']][i] += day['sleeping'][i]
	print(counts)
	maxes = dict()
...
def main():
	...
	d = sleepiest_minute()
	print(f'RESULT: {max(d.values())*int(keywithmaxval(d))}')
